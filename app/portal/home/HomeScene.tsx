...
const WALL_T   = 0.12
const FLOOR_T  = 0.08
const Y0       = 0           // ground floor Y
const Y1       = WALL_H + FLOOR_T  // first floor Y (~2.88)
...
const GROUND: RoomDef[] = [
  {
    ...
    items:[
      {rx:0.15, rz:0.15, w:3.5,  h:0.9,  d:0.55, col:C.dark},   // counter back
      {rx:0.15, rz:0.15, w:0.55, h:0.9,  d:2.0,  col:C.dark},   // counter left
      {rx:1.2,  rz:1.1,  w:1.6,  h:0.9,  d:0.85, col:C.white},  // island
    ],
  },
  {
    id:'eethoek', label:'Eethoek', x:4.7, z:0, w:3.8, d:4.3, floorY:Y0, floorCol:C.floorWood,
    items:[
      {rx:0.8, rz:1.1, w:2.2, h:0.75, d:1.1, col:C.table},       // table
      {rx:0.6, rz:0.4, w:0.5, h:0.45, d:0.5, col:C.white},       // chair N1
      {rx:1.3, rz:0.4, w:0.5, h:0.45, d:0.5, col:C.white},       // chair N2
      {rx:2.0, rz:0.4, w:0.5, h:0.45, d:0.5, col:C.white},       // chair N3
      {rx:0.6, rz:2.6, w:0.5, h:0.45, d:0.5, col:C.white},       // chair S1
      {rx:1.3, rz:2.6, w:0.5, h:0.45, d:0.5, col:C.white},       // chair S2
      {rx:2.0, rz:2.6, w:0.5, h:0.45, d:0.5, col:C.white},       // chair S3
    ],
  },
  {
    id:'zithoek', label:'Zithoek', x:4.7, z:4.3, w:3.8, d:5.1, floorY:Y0, floorCol:C.floorWood,
    items:[
      {rx:0.3, rz:0.8, w:2.6,  h:0.75, d:0.9, col:C.sofa},       // sofa main
      {rx:2.7, rz:0.8, w:0.9,  h:0.75, d:2.0, col:C.sofa},       // sofa arm
      {rx:0.8, rz:2.1, w:1.2,  h:0.45, d:0.65,col:C.table},      // coffee table
      {rx:0.3, rz:4.5, w:3.0,  h:0.35, d:0.35,col:C.dark},       // TV unit
      {rx:0.9, rz:4.0, w:1.8,  h:1.0,  d:0.08,col:0x1a1a2e},     // TV screen
    ],
  },
  {
    id:'berging', label:'Berging', x:0, z:3.0, w:3.8, d:2.5, floorY:Y0, floorCol:C.floorHall,
    items:[
      {rx:0.15,rz:0.3, w:0.6, h:0.85, d:0.6,  col:C.grey},       // washer
      {rx:0.85,rz:0.3, w:0.6, h:0.85, d:0.6,  col:C.grey},       // dryer
      {rx:1.8, rz:0.15,w:1.8, h:2.0,  d:0.35, col:C.shelves},    // shelving
    ],
  },
  {
    id:'garage', label:'Garage', x:0, z:5.5, w:5.0, d:4.1, floorY:Y0, floorCol:C.floorGarage,
    items:[
      {rx:0.5, rz:0.2, w:1.85,h:0.6,  d:3.9,  col:C.car},        // car body
      {rx:0.55,rz:0.55,w:1.75,h:0.35, d:1.8,  col:0x0a0e1e},     // car roof
    ],
  },
  {
  ...
  },
  {
    ...
    items:[
      {rx:0.1, rz:0.1, w:0.45,h:0.4,  d:0.55, col:C.white},      // toilet
      {rx:0.7, rz:0.1, w:0.5, h:0.85, d:0.4,  col:C.white},      // sink
    ],
  },
]
...
const FIRST: RoomDef[] = [
  {
    ...
    items:[
      {rx:0.15,rz:0.2, w:0.85,h:0.55, d:1.7,  col:C.white},      // bathtub
      {rx:0.15,rz:2.9, w:0.45,h:0.4,  d:0.55, col:C.white},      // toilet
      {rx:1.0, rz:2.75,w:1.3, h:0.85, d:0.5,  col:C.white},      // vanity
    ],
  },
  {
    id:'dressing', label:'Dressing', x:2.6, z:0, w:2.0, d:2.8, floorY:Y1, floorCol:C.floorWood,
    items:[
      {rx:0.1, rz:0.15,w:1.7, h:2.1,  d:0.55, col:C.dark},       // wardrobe
    ],
  },
  {
    id:'slaapkamer', label:'Slaapkamer', x:4.6, z:0, w:3.9, d:3.6, floorY:Y1, floorCol:C.floorWood,
    items:[
      {rx:0.7, rz:0.3, w:1.8, h:0.15, d:2.1,  col:0x3a2a18},     // bed base
      {rx:0.7, rz:0.3, w:1.8, h:0.55, d:2.1,  col:C.bed},        // mattress
      {rx:0.7, rz:0.3, w:1.8, h:1.0,  d:0.12, col:0x2a2040},     // headboard
      {rx:0.3, rz:0.3, w:0.4, h:0.5,  d:0.45, col:C.table},      // nightstand L
      {rx:2.7, rz:0.3, w:0.4, h:0.5,  d:0.45, col:C.table},      // nightstand R
      {rx:0.1, rz:2.9, w:2.2, h:2.1,  d:0.55, col:C.dark},       // wardrobe
      {rx:3.2, rz:0.3, w:0.6, h:0.75, d:1.6,  col:C.table},      // desk
      {rx:3.2, rz:1.0, w:0.6, h:0.45, d:0.6,  col:C.grey},       // chair
    ],
  },
  {
    id:'bureau', label:'Bureau', x:0, z:3.6, w:2.9, d:3.4, floorY:Y1, floorCol:C.floorWood,
    items:[
      {rx:0.2, rz:0.2, w:1.5, h:0.75, d:0.7,  col:C.table},      // desk
      {rx:0.4, rz:1.0, w:0.6, h:0.45, d:0.6,  col:C.grey},       // chair
      {rx:0.1, rz:2.6, w:2.5, h:2.1,  d:0.35, col:C.shelves},    // bookshelf
    ],
  },
  {
...
// ─── Light fixture positions (scene world coords) ─────────────
// key matches a "default" room id; real Hue lights override these via inferRoom()
...
// ─────────────────────────────────────────────────────────────
//  Three.js scene builder helpers
// ─────────────────────────────────────────────────────────────
...
function buildRoom(scene: THREE.Scene, r: RoomDef) {
  ...
  // 4 walls (share edge with neighbours – slight overlap is invisible from iso view)
  const wc = C.wallExt
  scene.add(box(r.w + WALL_T, WALL_H, WALL_T, wc, cx,       wy, r.z))           // N
  scene.add(box(r.w + WALL_T, WALL_H, WALL_T, wc, cx,       wy, r.z + r.d))     // S
  scene.add(box(WALL_T,       WALL_H, r.d,    wc, r.x,      wy, cz))             // W
  scene.add(box(WALL_T,       WALL_H, r.d,    wc, r.x + r.w,wy, cz))             // E
...
export default function HomeScene({ initialLights }: Props) {
  ...
  // ── Refresh lights from Bridge ────────────────────────────
  const refresh = useCallback(async () => {
    try {
      setLoading(true)
      const l = await fetchLights()
      setLights(l)
      setError(null)
    } catch (e) {
  ...
  // ── Sync Three.js point lights whenever lights map changes ─
  useEffect(() => {
    const pls = plightsRef.current
    for (const l of Object.values(lights)) {
      ...
      if (l.state.on && l.state.reachable !== false) {
        ...
        // Colour
        let hexCol = WARM_WHITE_HEX
        if (l.state.colormode === 'xy' && l.state.xy) {
          hexCol = xyBriToHex(l.state.xy, bri)
        }
        pl.color.set(hexCol)
      } else {
        pl.intensity = 0
  ...
  // ── Build / rebuild scene when floor changes ───────────────
  useEffect(() => {
    ...
    const resize = () => {
      ...
      if (camRef.current) {
        ...
        cam.left = -d * aspect; cam.right = d * aspect
    ...
    // Build rooms for active floor
    const rooms = floor === 'ground' ? GROUND : FIRST
    for (const r of rooms) scene.add(...[]), buildRoom(scene, r)

    // Build fixture spheres + point lights
    const pls = new Map<string, THREE.PointLight>()
    for (const [roomId, pos] of Object.entries(FIXTURE_POS)) {
      const onFirstFloor = pos.y > Y1
      if ((floor === 'ground' && onFirstFloor) || (floor === 'first' && !onFirstFloor)) continue
      ...
      // Point light (warm, off by default)
    ...
    // Sync current light state into point lights
    for (const l of Object.values(lights)) {
      const room = inferRoom(l.name)
      if (!room) continue
      const pl = pls.get(room)
      ...
      if (l.state.on && l.state.reachable !== false) {
        ...
        pl.intensity = (bri / 254) * 2.2
        if (l.state.colormode === 'xy' && l.state.xy) {
          pl.color.set(xyBriToHex(l.state.xy, bri))
        }
      }
    }
  ...
  const groundLights = lightList.filter(l => {
    const r = inferRoom(l.name)
    return r ? GROUND.some(g => g.id === r) : true
  })
  const firstLights = lightList.filter(l => {
    const r = inferRoom(l.name)
    return r ? FIRST.some(f => f.id === r) : false
  })
  ...
  return (
    <>
      <iframe
        ...
        title="background"
      />
      <div style={{ position:'fixed', inset:0, zIndex:1, display:'flex', flexDirection:'column', fontFamily:'var(--font-body)', color:'var(--ink)' }}>
        ...
        <div className="glass" style={{ height:72, padding:'0 30px', display:'flex', alignItems:'center', gap:16, borderBottom:'1px solid var(--edge)', flexShrink:0 }}>
            ...
            {(['ground','first'] as Floor[]).map(f => (
              <button key={f}
                ...
                style={{
                  ...
                  fontFamily:'var(--font-mono)', fontSize:11, cursor:'pointer', letterSpacing:'.1em',
          ...
          <button onClick={refresh} disabled={loading}
            style={{ padding:'7px 14px', borderRadius:'var(--r-pill)', border:'1px solid var(--edge)', background:'transparent', color:'var(--ink-3)', fontSize:11, fontFamily:'var(--font-mono)', cursor:'pointer', letterSpacing:'.1em', opacity: loading ? .5 : 1 }}>
            {loading ? '···' : '⟳ SYNC'}
          </button>
        </div>
          ...
          <div className="glass" style={{ width:280, display:'flex', flexDirection:'column', borderLeft:'1px solid var(--edge)', overflow:'hidden' }}>
            ...
            <div style={{ flex:1, overflowY:'auto', padding:'12px 0' }}>
              ...
              {activeList.map(l => {
                ...
                return (
                  <div key={l.id}
                    style={{ padding:'10px 20px', display:'flex', flexDirection:'column', gap:6, borderBottom:'1px solid rgba(160,195,255,.06)' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                      {/* Bulb indicator */}
                      <div style={{
                        width:10, height:10, borderRadius:'50%', flexShrink:0,
                        background: l.state.on ? col : '#1a1f2e',
                        boxShadow: l.state.on ? `0 0 8px ${col}` : 'none',
                        border:'1px solid rgba(160,195,255,.2)',
                      }}/>
                      <span style={{ flex:1, fontSize:13, fontWeight:500, color: l.state.on ? 'var(--ink)' : 'var(--ink-3)' }}>
                        {l.name}
                      </span>
                      {/* Toggle */}
                      <button
                        ...
                        style={{
                          ...
                          background: l.state.on ? 'rgba(150,190,255,.35)' : 'rgba(160,195,255,.08)',
                          position:'relative', transition:'background .2s', flexShrink:0,
                          opacity: toggling === l.id ? .5 : 1,
                        }}>
                        <div style={{
                          position:'absolute', top:3, left: l.state.on ? 19 : 3,
                          width:16, height:16, borderRadius:'50%',
                          background: l.state.on ? 'var(--ice)' : 'var(--ink-3)',
                          transition:'left .2s, background .2s',
                          boxShadow: l.state.on ? `0 0 8px var(--glow)` : 'none',
                        }}/>
                      </button>
                    </div>
                    {/* Brightness slider */}
                    {l.state.on && (
                      <input type="range" min={1} max={100} value={bri}
                        onChange={async e => {
                          ...
                          setLights(prev => ({
                            ...prev,
                            [l.id]: { ...prev[l.id], state: { ...prev[l.id].state, bri: Math.round(pct/100*254) } },
                          }))
                        }}
                    ...
                    {room && (
                      <span style={{ fontSize:10, color:'var(--ink-3)', fontFamily:'var(--font-mono)', letterSpacing:'.1em', textTransform:'uppercase' }}>
                        {room}
                      </span>
                    )}
                  </div>
              ...
              {unassigned.length > 0 && (
                <>
                  <div style={{ padding:'10px 20px 4px', fontSize:10, color:'var(--ink-3)', fontFamily:'var(--font-mono)', letterSpacing:'.12em', textTransform:'uppercase' }}>
                    Niet toegewezen
                  </div>
                  {unassigned.map(l => (
                    <div key={l.id} style={{ padding:'10px 20px', display:'flex', alignItems:'center', gap:10, borderBottom:'1px solid rgba(160,195,255,.06)', opacity:.7 }}>
                      ...
                      <button disabled={toggling === l.id} onClick={() => handleToggle(l)}
                        style={{ fontSize:11, padding:'4px 10px', borderRadius:'var(--r-pill)', border:'1px solid var(--edge)', background:'transparent', color:'var(--ink-3)', cursor:'pointer' }}>
                        {l.state.on ? 'Uit' : 'Aan'}
                      </button>
                    </div>
                  ))}
