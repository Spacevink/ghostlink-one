...
const FC = {
  ...
  bedLinen:   0x364a70,
  bedSheet:   0xd0d8e8,
  bookA:      0x4a3418, bookB: 0x2a3a5a, bookC: 0x5a2a2a, bookD: 0x2a4a2a,
  car:        0x1e3a70,
  carRoof:    0x0a1830,
  glass:      0x88aabb,
  rug:        0x8a7060,
...
function makeHerringbone(): THREE.CanvasTexture {
  ...
  let n = 0
  const plank = (x:number,y:number,w:number,h:number,hz:boolean) => {
    ctx.fillStyle = pal[(n++)%pal.length]; ctx.fillRect(x,y,w,h)
    ctx.strokeStyle='rgba(30,15,0,.28)'; ctx.lineWidth=1.5
  ...
  for (let row=-1; row*PL<S+PL*2; row++)
    for (let col=-2; col*PW*2<S+PW*4; col++) {
      const off=(row%2!==0)?PW:0, ox=col*PW*2+off, oy=row*PL
      plank(ox,      oy,    PL,PW,true)
      plank(ox+PL-PW,oy+PW, PW,PL,false)
    }
...
function makeTile(): THREE.CanvasTexture {
  const S=256,T=32; const cv=document.createElement('canvas'); cv.width=cv.height=S
  const ctx=cv.getContext('2d')!
  ctx.fillStyle='#c4ccd8'; ctx.fillRect(0,0,S,S)
...
function makeConcrete(): THREE.CanvasTexture {
  ...
  const ctx=cv.getContext('2d')!
  ctx.fillStyle='#888680'; ctx.fillRect(0,0,S,S)
...
function makeHall(): THREE.CanvasTexture {
  const S=256,T=48; const cv=document.createElement('canvas'); cv.width=cv.height=S
  const ctx=cv.getContext('2d')!
  ctx.fillStyle='#7a7068'; ctx.fillRect(0,0,S,S)
  for (let r=0;r<S/T;r++) for (let c=0;c<S/T;c++) {
...
const GROUND: RoomDef[] = [
  ...
  { id:'zithoek', label:'Zithoek', x:4.7, z:4.3, w:3.8, d:5.1, floorY:Y0, tex:'wood', items:[
      ...
      {rx:.22,rz:.65,w:2.85,h:1.0,d:.12,col:FC.sofaBack},
  ...
  { id:'berging', label:'Berging', x:0, z:3.0, w:3.8, d:2.5, floorY:Y0, tex:'hall', items:[
      ...
      {rx:2.0,rz:.1,w:1.6,h:2.2,d:.38,col:0x282830,cast:true},
  ]},
  { id:'garage', label:'Garage', x:0, z:5.5, w:5.0, d:4.1, floorY:Y0, tex:'concrete', items:[
      ...
      {rx:.65,rz:.9,w:1.7,h:.38,d:2.0,col:FC.carRoof,cast:true},
  ...
  { id:'wc', label:'WC', x:5.6, z:8.2, w:1.4, d:1.0, floorY:Y0, tex:'tile', items:[
      ...
      {rx:.72,rz:.08,w:.55,h:.04,d:.42,col:0xe8e4dc},
  ]},
]
...
const FIRST: RoomDef[] = [
  { id:'badkamer', label:'Badkamer', x:0, z:0, w:2.6, d:3.6, floorY:Y1, tex:'tile', items:[
      ...
      {rx:.8,rz:2.7,w:1.6,h:.04,d:.52,col:0xe0dcd4},
  ...
  { id:'dressing', label:'Dressing', x:2.6, z:0, w:2.0, d:2.8, floorY:Y1, tex:'wood', items:[
      ...
      {rx:.08,rz:1.88,w:.04,h:2.4,d:.58,col:0x282830},
  ]},
  ...
  { id:'bureau', label:'Bureau', x:0, z:3.6, w:2.9, d:3.4, floorY:Y1, tex:'wood', items:[
      ...
      {rx:.12,rz:2.6,w:.5,h:1.9,d:.3,col:FC.bookA},
      {rx:.72,rz:2.6,w:.5,h:1.9,d:.3,col:FC.bookB},
      {rx:1.32,rz:2.6,w:.5,h:1.9,d:.3,col:FC.bookC},
      {rx:1.92,rz:2.6,w:.5,h:1.9,d:.3,col:FC.bookD},
  ]},
...
const FIXTURE_POS: Record<string, {x:number; y:number; z:number}> = {
  keuken:     {x:2.15, y:Y0+WALL_H-.15, z:1.5},
  eethoek:    {x:6.3,  y:Y0+WALL_H-.15, z:2.0},
  zithoek:    {x:6.3,  y:Y0+WALL_H-.15, z:7.0},
  berging:    {x:1.9,  y:Y0+WALL_H-.15, z:4.2},
  inkom:      {x:4.7,  y:Y0+WALL_H-.15, z:8.7},
  garage:     {x:2.5,  y:Y0+WALL_H-.15, z:7.5},
  slaapkamer: {x:6.5,  y:Y1+WALL_H-.15, z:1.8},
  badkamer:   {x:1.3,  y:Y1+WALL_H-.15, z:1.8},
  dressing:   {x:3.6,  y:Y1+WALL_H-.15, z:1.4},
  bureau:     {x:1.4,  y:Y1+WALL_H-.15, z:5.3},
...
function buildRoom(scene:THREE.Scene, r:RoomDef, tex:Record<TexKey,THREE.Texture>) {
  ...
  for (const [x,y,z,w,h,d] of [
    ...
    [cx,       wy, r.z+r.d,  r.w+WALL_TI, WALL_H, WALL_TI],
    [r.x,      wy, cz,       WALL_TI, WALL_H, r.d],
...
function buildExterior(scene:THREE.Scene, floorY:number) {
  const m=new THREE.MeshLambertMaterial({color:EXT_COL}), wy=floorY+WALL_H/2
  const HW=8.5, HD=9.6, T=WALL_T
  for (const [x,y,z,w,h,d] of [
    [HW/2, wy, 0,    HW+T*2, WALL_H, T],
    [HW/2, wy, HD,   HW+T*2, WALL_H, T],
    [0,    wy, HD/2, T, WALL_H, HD+T*2],
    [HW,   wy, HD/2, T, WALL_H, HD+T*2],
  ] as [number,number,number,number,number,number][])
    put(scene, new THREE.BoxGeometry(w,h,d), m, x,y,z, false, true)
}

function buildStairs(scene:THREE.Scene) {
  const m=new THREE.MeshLambertMaterial({color:0x9a8870})
  const sX=4.15, sZ=7.3, sW=1.3, stepD=0.20, stepH=0.20, n=14
  for (let i=0;i<n;i++) {
    const h=stepH*(i+1)
    put(scene, new THREE.BoxGeometry(sW,h,stepD), m,
      sX+sW/2, h/2, sZ-i*stepD-stepD/2, true, true)
  }
  const wm=new THREE.MeshLambertMaterial({color:INT_COL})
  put(scene, new THREE.BoxGeometry(WALL_TI,WALL_H,n*stepD+.2), wm, sX,WALL_H/2, sZ-n*stepD/2)
  put(scene, new THREE.BoxGeometry(WALL_TI,WALL_H,n*stepD+.2), wm, sX+sW,WALL_H/2, sZ-n*stepD/2)
}

function buildGarden(scene:THREE.Scene, concreteTex:THREE.Texture) {
  const m=new THREE.MeshLambertMaterial({map:concreteTex})
  put(scene, new THREE.BoxGeometry(3.8,FLOOR_T,8.0), m, -1.9,Y0-FLOOR_T/2,4.0, false,true)
  const lm=new THREE.MeshLambertMaterial({color:0x3a3a42})
  put(scene, new THREE.BoxGeometry(3.8,.25,.15), lm, -1.9,.125,.075)
  put(scene, new THREE.BoxGeometry(3.8,.25,.15), lm, -1.9,.125,7.925)
...
export default function HomeScene({ initialLights }: Props) {
  ...
  useEffect(()=>{
    ...
    for (const [roomId,pos] of Object.entries(FIXTURE_POS)) {
      ...
      const pl=new THREE.PointLight(0xffd49a,0,9,1.5)
      pl.position.set(pos.x,pos.y-.25,pos.z); scene.add(pl)
      pls.set(roomId,pl)
    }
    ...
    for (const l of Object.values(lights)) {
      ...
      const bri=l.state.bri??200; pl.intensity=(bri/254)*4.5
            ...
            {active.map(l=>{
              ...
              return (
                <div key={l.id} style={{padding:'10px 20px',display:'flex',flexDirection:'column',gap:6,borderBottom:'1px solid rgba(160,195,255,.06)'}}>
                  ...
                  {l.state.on&&<input type="range" min={1} max={100} value={bri}
                    onChange={async e=>{ const p=Number(e.target.value); await setBrightness(l.id,p); setLights(prev=>({...prev,[l.id]:{...prev[l.id],state:{...prev[l.id].state,bri:Math.round(p/100*254)}}})) }}
